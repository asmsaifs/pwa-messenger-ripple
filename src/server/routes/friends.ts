...
friendsRoute.post('/invite', async (c) => {
  ...
  if (targetUser && targetUser.id !== actor.userId) {
    ...
    if (existing?.status === 'declined') {
      await friendsRepo.reviveFriendshipRequest(c.env, actor, existing.id);
    } else if (!existing) {
      await friendsRepo.createFriendshipRequest(c.env, actor, targetUser.id);
    }
    // an existing `pending` row means the request already stands — idempotent no-op
