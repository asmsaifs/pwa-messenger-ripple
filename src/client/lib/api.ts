// Relative, not the `@shared` alias: this module is also imported (via
// outbox.ts) from src/sw.ts, which the PWA plugin builds as its own bundle
// with no alias resolution configured — a relative path resolves in both.
